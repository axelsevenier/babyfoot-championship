...
// ─── State ────────────────────────────────────────────────────────────────────
let state = { joueurs: [...DEFAULT_JOUEURS], matchs: [...DEFAULT_MATCHS] };
let activeMois = new Date().getMonth();

// ─── Persistence ──────────────────────────────────────────────────────────────
function save() {
  try { localStorage.setItem('bf_state', JSON.stringify(state)); } catch (e) {}
}

function load() {
  try {
    const d = localStorage.getItem('bf_state');
...
function adjustScore(id, delta) {
  ...
  const val = Math.max(0, Math.min(20, parseInt(el.value || 0) + delta));
  el.value = val;
}
...
function saveMatch() {
  ...
  if (ba === bb) {
    msg.textContent = '⚠ Pas de match nul (scores différents).';
    msg.className = 'save-msg error';
    return;
  }

  state.matchs.push({ mois, a1, a2, b1, b2, ba, bb, date });
  save();
...
function renderMensuel(mois) {
  ...
  pillsEl.innerHTML = MOIS_NOMS.map((m, i) => {
  ...
  // Mini stats
  const totalMatchs = matchsMois.length;
  const joueursMois = [...new Set(matchsMois.flatMap(m => [m.a1,m.a2,m.b1,m.b2]))];

  content.innerHTML = `
    <div style="display:flex;gap:.75rem;margin-bottom:1rem;flex-wrap:wrap">
      <div class="duo-stat-card" style="flex:1;min-width:100px">
        <div class="duo-stat-label">Matchs joués</div>
        <div class="duo-stat-value">${totalMatchs}</div>
      </div>
      <div class="duo-stat-card" style="flex:1;min-width:100px">
        <div class="duo-stat-label">Joueurs actifs</div>
        <div class="duo-stat-value">${joueursMois.length}</div>
      </div>
      <div class="duo-stat-card" style="flex:1;min-width:100px">
        <div class="duo-stat-label">Classés</div>
        <div class="duo-stat-value">${ranked.length}</div>
      </div>
    </div>
    <div class="card">
...
function deleteLastMatch() {
  if (!state.matchs.length) return;
  const last = state.matchs[state.matchs.length - 1];
  const confirm_msg = `Supprimer le dernier match ?\n${last.a1} & ${last.a2} ${last.ba}–${last.bb} ${last.b1} & ${last.b2}`;
...
window.deleteLastMatch = deleteLastMatch;
...
function renderDuos() {
  ...
  for (let i = 0; i < joueurs.length; i++) {
    for (let k = i + 1; k < joueurs.length; k++) {
      ...
      const totalPairs = (joueurs.length - 1);
      const avgPerDuo = totalPairs > 0 ? matchs.length / totalPairs : 0;
  ...
  document.getElementById('duo-stats-grid').innerHTML = `
    <div class="duo-stat-card">
      <div class="duo-stat-label">Total duos</div>
      <div class="duo-stat-value">${totalDuos}</div>
    </div>
    <div class="duo-stat-card">
      <div class="duo-stat-label">Duos à 0 match</div>
      <div class="duo-stat-value" style="${duos0 > 0 ? 'color:#dc2626' : ''}">${duos0}</div>
    </div>
    <div class="duo-stat-card">
      <div class="duo-stat-label">Moy. matchs/duo</div>
      <div class="duo-stat-value">${avgMatchs}</div>
    </div>`;
  ...
  document.getElementById('tbody-duos').innerHTML = duos.map(d => {
    const prio = d.n === 0
      ...
      : d.ecart < -0.5
        ? `<span class="badge-rattraper">Rattraper</span>`
        : '—';
    return `<tr>
      <td><strong>${d.j1}</strong> & <strong>${d.j2}</strong></td>
      <td class="col-num">${d.n}</td>
      <td class="col-num">${d.ecart > 0 ? '+' : ''}${d.ecart}</td>
      <td>${prio}</td>
    </tr>`;
  }).join('');
}
